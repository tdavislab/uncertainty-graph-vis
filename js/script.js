...
function renderCoarseEgo(uncert,file){
    ...
    //Creates k bar
    let k_Bar = new kBar(this.k,range,'coarse-ego');
    ...
    if (uncert == 'co_occurrence'){
        ...
        //Creates f bar
        let f_Bar = new fBar(this.f,f_range,'ego-filter');
        ...
        //detects changes on k bar and changes files accordingly 
        d3.select('#coarse-ego').on('input', function(d){
            let new_k = k_Bar.activeK;
            let new_q = q_Bar.active;
            populateStuff(new_k,new_q,uncert,file)
        ...
        // FILTER BAR FUNCTIONALITY
        // Detects changes and messes with graph edge visibility
        d3.select('#ego-filter').on('input', function(d){
            that.f = f_Bar.activeF
            // console.log(that.f)
        ...
        function populateStuff(k,q,uncert,file){
            Promise.all([
                // Q_graph
                d3.json(`data/${file}/ego_572/cluster_${k}/${uncert}/Q_graph/Q_graph_${q}.json`),
                // individual instances
                d3.json(`data/${file}/ego_572/cluster_${k}/${uncert}/individual_instances/clustered_graph_${q}.json`),
                // Q_matrix
                d3.csv(`data/${file}/ego_572/cluster_${k}/${uncert}/Q_matrix/Q_mat_${q}.csv`),
                // Also need to load a representative original graph 
                // d3.json(`data/${file}/football_115/cluster_${k}/local_adjusted_rand_index/ori_graph_with_cluster.json`)
                // Instead, I'm loading a - star data
                d3.json(`data/${file}/ego_572/cluster_${k}/${uncert}/a_star_graph.json`)
                ...
                heatMap.coOccur = true;
                heatMap.active_alg = file;
                heatMap.data_name = 'ego_572'; // Maybe I need to change this?
                heatMap.uncert = uncert;
                heatMap.k = k;
                heatMap.createHeatMap()
                // Pass references to heatmap as well
        ...
        Promise.all([
            //reduced
            d3.json(`data/${file}/ego_572/cluster_${k}/${uncert}/uncertainty_graph.json`),
            //original
            d3.json(`data/${file}/ego_572/cluster_${k}/${uncert}/ori_graph_with_cluster.json`),
            // uncertainty matrix
            d3.csv(`data/${file}/ego_572/cluster_${k}/${uncert}/uncertain_mat.csv`)
